...
let customConfigurationProviders = [];
...
function registerCustomConfigurationProvider(provider) {
    customConfigurationProviders.push(provider);
    if (realActivationOccurred) {
        onDidChangeCustomConfiguration(provider);
    }
}
exports.registerCustomConfigurationProvider = registerCustomConfigurationProvider;
function provideCustomConfiguration(document, client) {
    return __awaiter(this, void 0, void 0, function* () {
        let tokenSource = new vscode_jsonrpc_1.CancellationTokenSource();
        if (customConfigurationProviders.length === 0) {
            return Promise.resolve();
        }
        return client.runBlockingThenableWithTimeout(() => __awaiter(this, void 0, void 0, function* () {
            for (let i = 0; i < customConfigurationProviders.length; i++) {
                if (yield customConfigurationProviders[i].canProvideConfiguration(document.uri)) {
...
exports.provideCustomConfiguration = provideCustomConfiguration;
function onDidChangeCustomConfiguration(customConfigurationProvider) {
    clients.forEach((client) => client.onDidChangeCustomConfigurations(customConfigurationProvider));
}
exports.onDidChangeCustomConfiguration = onDidChangeCustomConfiguration;
