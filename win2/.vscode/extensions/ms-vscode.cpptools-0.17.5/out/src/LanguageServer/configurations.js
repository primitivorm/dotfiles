...
class CppProperties {
    ...
    applyDefaultIncludePathsAndFrameworks() {
        if (this.configurationIncomplete && this.defaultIncludes && this.defaultFrameworks && this.vcpkgPathReady) {
            let configuration = this.configurationJson.configurations[this.CurrentConfiguration];
    ...
    buildVcpkgIncludePath() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (yield util.checkFileExists(util.getVcpkgPathDescriptorFile())) {
                    let vcpkgRoot = yield util.readFileText(util.getVcpkgPathDescriptorFile());
                    vcpkgRoot = vcpkgRoot.trim();
                    if (yield util.checkDirectoryExists(vcpkgRoot)) {
                        let vcpkgInstalledPath = path.join(vcpkgRoot, "/installed");
                        let list = yield util.readDir(vcpkgInstalledPath);
                        if (list !== undefined) {
                            list.forEach((entry) => {
                                if (entry !== "vcpkg") {
                                    let pathToCheck = path.join(vcpkgInstalledPath, entry);
